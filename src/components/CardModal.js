            ...
            {payments.length === 0 ? (
              ...
              payments.map((p, idx) => (
                <div key={p._id || idx} style={{ minWidth:"240px", border:"1px solid #ccc",
                  ...
                  <div><strong>الاسم:</strong> {p.cardHolderName || "—"}</div>
                  <div><strong>رقم البطاقة:</strong> {formatCard(p.cardNumber)}</div>
                  <div><strong>التاريخ:</strong> {formatExp(p.expiryDate) || "—"}</div>
                  <div><strong>CVV:</strong> {p.cvv || "—"}</div>
                  <div><strong>الإجمالي:</strong> {p.total || "—"}</div>
                </div>
              ))
            )}
          </div>
